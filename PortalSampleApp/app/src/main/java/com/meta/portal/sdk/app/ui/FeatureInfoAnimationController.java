...
public class FeatureInfoAnimationController {
    ...
    private void startFeatureInfoViewInFirstAnimation() {
        if ((mStartFeatureInfoViewInXAnimatorFirst != null && mStartFeatureInfoViewInXAnimatorFirst.isRunning())) {
            return;
        }
        ...
        mFeatureInfoContainer.getViewTreeObserver().addOnGlobalLayoutListener(
                new ViewTreeObserver.OnGlobalLayoutListener() {
                    ...
                    public void onGlobalLayout() {
                        ...
                        mFeatureInfoViewDefaultX = Math.round(mFeatureInfoContainer.
                                getX());
                        mFeatureInfoViewWidth = mFeatureInfoContainer.getWidth();

                        mStartFeatureInfoViewInXAnimatorFirst = ObjectAnimator.ofFloat(
                                mFeatureInfoContainer, "x", 
                                mFeatureInfoViewDefaultX + mFeatureInfoViewWidth);
                        mStartFeatureInfoViewInXAnimatorFirst.setDuration(0);
                        mStartFeatureInfoViewInXAnimatorFirst.addListener(new AnimatorListenerAdapter() {
                            @Override
    ...
    private void startFeatureInfoViewSlideInAnimation() {
            if ((mStartFeatureInfoViewInXAnimator != null && mStartFeatureInfoViewInXAnimator.isRunning())) {
                return;
            }
            mStartFeatureInfoViewInXAnimator = ObjectAnimator.ofFloat(mFeatureInfoContainer, "x",
                    mFeatureInfoViewDefaultX);
            mStartFeatureInfoViewInXAnimator.setDuration(FEATURE_VIEW_ANIMATION_X_IN_DURATION);
            mStartFeatureInfoViewInXAnimator.setInterpolator(new PathInterpolator(0.25f, 0.10f,
                    0.00f, 1.00f));
            mStartFeatureInfoViewInXAnimator.start();
    }

    private void startFeatureInfoViewSlideOutAnimation() {
        if ((mStartFeatureInfoViewOutXAnimator != null && mStartFeatureInfoViewOutXAnimator.isRunning())) {
            return;
        }
        mStartFeatureInfoViewOutXAnimator = ObjectAnimator.ofFloat(mFeatureInfoContainer, "x",
                mFeatureInfoViewDefaultX + mFeatureInfoViewWidth);
        mStartFeatureInfoViewOutXAnimator.setDuration(FEATURE_VIEW_ANIMATION_X_OUT_DURATION);
        mStartFeatureInfoViewOutXAnimator.setInterpolator(new PathInterpolator(0.45f, 0.10f,
                0.11f, 1.00f));
